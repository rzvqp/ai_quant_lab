"""Phase 10 BTCUSD Operational Infrastructure Test -- CEO-authorized 2026-07-25.

NOT a strategy test. NOT a performance test. Validates EXCLUSIVELY the execution infrastructure path:
AI Trader -> Execution Orchestrator -> Order Manager -> Broker Adapter -> MT5 order_check ->
MT5 order_send -> confirmation -> controlled close -> report.

BTCUSD is used instead of XAUUSD purely because the XAUUSD/metals market is closed on weekend and
BTCUSD trades 24/7 -- this lets the send path actually be exercised today. This script imports and
calls ONLY existing, already-approved `ai_trader` code, unmodified -- it lives at repo root (mirroring
the established `mt5_connectivity_probe.py` precedent), not inside any `ai_trader` package, and is not
part of the standing automated test suite.

Mandatory pre-send checks (all 12, CEO's own explicit list) are logged individually before any order.
Any failure aborts immediately, fail-closed, no order sent. On success: exactly one DEMO order at the
broker's own minimum volume, confirmed, then immediately closed in a controlled manner, then verified
that no position/order remains open from this test.
"""
...
SYMBOL = "BTCUSD"
JOURNAL_PATH = Path(__file__).parent / "btcusd_phase10_operational_test_journal.jsonl"
...
def main() -> None:
    ...
    try:
        ...
        log("CHECK_2_ACCOUNT_IS_DEMO", account_is_demo=status.account_is_demo, trade_mode=str(status.account_trade_mode))
        ...
        log("CHECK_8_TICK", tick_present=tick is not None, bid=getattr(tick, "bid", None), ask=getattr(tick, "ask", None), tick_time=getattr(tick, "time", None))
        if tick is None:
            abort("no tick data available for BTCUSD")
        ...
        # ---- Check 9+10: properties + spread available ----
        spread_price = float(tick.ask) - float(tick.bid)
        log("CHECK_9_10_SPREAD", bid=tick.bid, ask=tick.ask, spread_price=spread_price, broker_reported_spread_points=caps.spread)
        if spread_price <= 0:
        ...
        # Verify no pre-existing BTCUSD position before we start (a clean baseline for the "no leftover
        # positions" check at the end).
        ...
        if len(pre_positions) > 0:
            abort(f"{len(pre_positions)} pre-existing BTCUSD position(s) found before the test even started -- refusing to add ambiguity")
        ...
        risk_config.filters.reference_spread[SYMBOL] = max(spread_price * 3, 1.0)
        ...
        risk_context = RiskContext(as_of=now, per_symbol={SYMBOL: SymbolRiskSnapshot(
            atr=spread_price * 20, atr_rolling_median=spread_price * 20, current_spread=spread_price,
        ...
        log(
            ...
            state=order_result.state.value, filled_qty=order_result.filled_qty, avg_price=order_result.avg_price,
            dry_run=order_result.dry_run, execution_wall_clock=time.time(),
        )
        ...
        if len(open_positions) == 0:
            ...
            for position in open_positions:
                ...
                close_request = {
                    ...
                    "comment": "PHASE10-BTC-INFRA-CLOSE", "type_time": 0, "type_filling": 1,
                }
                ...
                log("CLOSE_ORDER_SEND", ticket=ticket, retcode=getattr(send_result, "retcode", None),
                    comment=getattr(send_result, "comment", None), close_price=getattr(send_result, "price", None),
