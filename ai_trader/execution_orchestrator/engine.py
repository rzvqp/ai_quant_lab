...
from ai_trader.risk_manager_live.types import LiveRiskDecision, TradeProposal
...
def orchestrate(
    ...
    if mode is ExecutionMode.LIVE:
        result = _denied(candidate, correlation_id, mode, trace, [rc.LIVE_TRADING_NOT_AUTHORIZED])
    ...
    if mode is ExecutionMode.DEMO:
        ...
        if not demo_ok:
            return _denied(candidate, correlation_id, mode, trace, [rc.NON_DEMO_ACCOUNT_REFUSED])
    ...
    if not staleness_ok:
        return _denied(candidate, correlation_id, mode, trace, [rc.STALE_CANDIDATE])
    ...
    try:
        ...
        return _denied(candidate, correlation_id, mode, trace, [rc.CONTEXT_BUILD_FAILED])
    ...
    try:
        ...
        return _denied(candidate, correlation_id, mode, trace, [rc.CONFIDENCE_ASSESSMENT_FAILED], context, recognition)
    ...
    if not confidence.eligible_for_risk_evaluation:
        result = _denied(
            ...
            context, recognition, confidence,
        )
    ...
    try:
        ...
        return _denied(candidate, correlation_id, mode, trace, [rc.RISK_EVALUATION_FAILED], context, recognition, confidence)

    if not risk_decision.approved:
        result = _denied(
            candidate, correlation_id, mode, trace, [rc.RISK_DENIED, *risk_decision.reason_codes],
            context, recognition, confidence, risk_decision,
        )
