...
from ai_trader.context_memory import OutcomeKind, aggregate_evidence
...
def test_explain_evidence_sufficient(tmp_path: Path) -> None:
    ...
    result = retrieve(idx, query)
    evidence = aggregate_evidence(idx, result, "S1", OutcomeKind.STRATEGY)
...
def test_explain_evidence_unavailable_zero_resolved(tmp_path: Path) -> None:
    ...
    result = retrieve(idx, query)
    evidence = aggregate_evidence(idx, result, "S7", OutcomeKind.STRATEGY)
    lines = explain_evidence(evidence)
