"""RangeSemanticEngineV44 -- compune N1 (0.1.1, NEATINS) + `RangeSemanticProducerV44` cu `ConfigV44`.
Mirror direct al `range_engine_v4_3.py` (nicio linie din N1 reimplementata, ATR canonic importat din N1
neschimbat) -- doar tipurile V4.3->V4.4 difera. Aditiv, izolat, fara wheel (mandat
VE-RANGE-V4_4-IMPLEMENTATION-001)."""
...
from .range_semantic_v4_4 import (
    ConfigV44, ConfigNotRatifiedErrorV43, ContractErrorV43, RangeSemanticProducerV44,
    RangeSemanticResultV44, RangeEventV43, SNAPSHOT_CONTRACT_MISMATCH,
    RANGE_HIERARCHICAL_V4_4_NORMATIVE_CONFIG_ID,
)
...
class RangeLedgerV44:
    ...
    records: tuple[RangeReplayRecordV44, ...]
...
class RangeSnapshotErrorV44(Exception):
    pass


class RangeSemanticEngineV44:
    ...
    (aceeasi disciplina ca `RangeSemanticEngineV43`)."""

    def __init__(self, *, symbol: str, timeframe: str, bar_interval_seconds: int,
                 implementation_commit: str, range_config: ConfigV44,
        ...
        if not acknowledge_construction_only:
            raise ConfigNotRatifiedErrorV43(
                "RangeSemanticEngineV44 e ADITIV, NEIMPLEMENTAT-IN-PRODUCTIE -- constructia refuza fara "
        ...
        if range_config.config_id() != RANGE_HIERARCHICAL_V4_4_NORMATIVE_CONFIG_ID:
            raise ContractErrorV43(
                f"CONFIG_ID_MISMATCH: {range_config.config_id()} != normativ "
                f"{RANGE_HIERARCHICAL_V4_4_NORMATIVE_CONFIG_ID}")
        ...
        self._range = RangeSemanticProducerV44(self._cfg)
        self._symbol = symbol
    ...
    @property
    def range_config(self) -> ConfigV44:
        return self._cfg
    ...
    def replay_batch(self, bars: Any, *, as_of: int | None = None) -> RangeLedgerV44:
        records: list[RangeReplayRecordV44] = []
        ...
        for bar in bars:
            ...
            records.append(RangeReplayRecordV44(
    ...
    def snapshot(self) -> RangeSnapshotV44:
        return RangeSnapshotV44(
    ...
    def restore(self, snapshot: Any) -> None:
        if not isinstance(snapshot, RangeSnapshotV44):
            raise RangeSnapshotErrorV44(
                f"refuz: snapshot de tip {type(snapshot).__name__!r}, nu RangeSnapshotV44 -- nicio migrare "
                "implicita de la V4.3 sau alta versiune, niciun rebuild partial acceptat")
        if snapshot.contract_version != self._cfg.contract_version or snapshot.config_id != self._cfg.config_id():
            raise RangeSnapshotErrorV44(SNAPSHOT_CONTRACT_MISMATCH)
        if snapshot.n1_identity_fingerprint != self._n1.identity.fingerprint():
            raise RangeSnapshotErrorV44("identitate N1 incompatibila")
        ...
        try:
            ...
            raise RangeSnapshotErrorV44(f"snapshot range_state corupt/incomplet: {exc!r}") from exc
