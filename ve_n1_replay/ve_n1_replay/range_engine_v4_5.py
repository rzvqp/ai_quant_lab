"""RangeSemanticEngineV45 -- composes N1 (0.1.1, UNTOUCHED) + `RangeSemanticProducerV45` with `ConfigV45`.
Direct mirror of `range_engine_v4_4.py` (no N1 line reimplemented, canonical ATR imported from N1
unchanged) -- only the V4.4->V4.5 types differ. Additive, isolated, no wheel bump (mandate
`VE-RANGE-V4-5-STALE-CANDIDATE-RECOVERY-001`)."""
...
from .range_semantic_v4_5 import (
    ConfigV45, ConfigNotRatifiedErrorV43, ContractErrorV43, RangeSemanticProducerV45,
...
class RangeLedgerV45:
    ...
    records: tuple[RangeReplayRecordV45, ...]
...
class RangeSnapshotErrorV45(Exception):
    pass


class RangeSemanticEngineV45:
    ...
    and VALIDATED in an ISOLATED producer before either N1 or `self._range` is touched (same discipline
    as `RangeSemanticEngineV44`/`RangeSemanticEngineV43`)."""

    def __init__(self, *, symbol: str, timeframe: str, bar_interval_seconds: int,
                 implementation_commit: str, range_config: ConfigV45,
        ...
        if not acknowledge_construction_only:
            raise ConfigNotRatifiedErrorV43(
                "RangeSemanticEngineV45 is ADDITIVE, NOT-IMPLEMENTED-IN-PRODUCTION -- construction refuses "
                "without explicit acknowledge_construction_only=True (mandate: no Strategy Catalog/Alpha/"
                "AI Trader/LIVE_SHADOW/broker promotion authorized by this mandate; candidate engineering "
                "artifact requiring independent validation before any ratification)")
        if range_config.config_id() != RANGE_HIERARCHICAL_V4_5_NORMATIVE_CONFIG_ID:
            raise ContractErrorV43(
                f"CONFIG_ID_MISMATCH: {range_config.config_id()} != normative "
                f"{RANGE_HIERARCHICAL_V4_5_NORMATIVE_CONFIG_ID}")
        ...
        self._range = RangeSemanticProducerV45(self._cfg)
        self._symbol = symbol
    ...
    @property
    def range_config(self) -> ConfigV45:
        return self._cfg
    ...
    def replay_batch(self, bars: Any, *, as_of: int | None = None) -> RangeLedgerV45:
        records: list[RangeReplayRecordV45] = []
        ...
        for bar in bars:
            ...
            records.append(RangeReplayRecordV45(
    ...
    def snapshot(self) -> RangeSnapshotV45:
        return RangeSnapshotV45(
    ...
    def restore(self, snapshot: Any) -> None:
        if not isinstance(snapshot, RangeSnapshotV45):
            raise RangeSnapshotErrorV45(
                f"refusing: snapshot of type {type(snapshot).__name__!r}, not RangeSnapshotV45 -- no "
                "implicit migration from V4.4 or any other version, no partial rebuild accepted")
        if snapshot.contract_version != self._cfg.contract_version or snapshot.config_id != self._cfg.config_id():
            raise RangeSnapshotErrorV45(SNAPSHOT_CONTRACT_MISMATCH)
        if snapshot.n1_identity_fingerprint != self._n1.identity.fingerprint():
            raise RangeSnapshotErrorV45("incompatible N1 identity")
        ...
        try:
            ...
            raise RangeSnapshotErrorV45(f"corrupt/incomplete range_state snapshot: {exc!r}") from exc
