"""RangeSemanticEngineV441 -- compune N1 (0.1.1, NEATINS) + `RangeSemanticProducerV441` cu `ConfigV441`.
Mirror direct al `range_engine_v4_4.py` (nicio linie din N1 reimplementata, ATR canonic importat din N1
neschimbat) -- doar tipurile V4.4->V4.4.1 difera. Aditiv, izolat, fara wheel (mandat
VE-RANGE-V4_4_1-STALE-IMPLEMENTATION-001). `range_semantic_v4_4.py`/`range_engine_v4_4.py` raman
byte-neatinse -- niciun tip din acest fisier nu le modifica, doar le importa (`RangeSemanticResultV44`,
reutilizat NESCHIMBAT -- T-STALE raporteaza prin campurile `macro_reason`/`macro_state` deja existente, niciun
camp nou necesar la nivel de rezultat)."""
...
from .range_semantic_v4_3 import ConfigNotRatifiedErrorV43, ContractErrorV43, RangeEventV43, SNAPSHOT_CONTRACT_MISMATCH
from .range_semantic_v4_4 import RangeSemanticResultV44
from .range_semantic_v4_4_1 import (
    ConfigV441, RangeSemanticProducerV441, RANGE_HIERARCHICAL_V4_4_1_NORMATIVE_CONFIG_ID,
)
...
class RangeLedgerV441:
    ...
    records: tuple[RangeReplayRecordV441, ...]
...
class RangeSnapshotErrorV441(Exception):
    pass


class RangeSemanticEngineV441:
    """Refuza fail-closed: `config_id` nepotrivit, `acknowledge_construction_only` lipsa, snapshot de alt
    tip/versiune/config/amprenta N1. Nicio migrare implicita, niciun rebuild partial -- starea noua e
    construita si VALIDATA intr-un producator IZOLAT inainte ca N1 sau `self._range` sa fie atinse
    (aceeasi disciplina ca `RangeSemanticEngineV43`/`RangeSemanticEngineV44`)."""

    def __init__(self, *, symbol: str, timeframe: str, bar_interval_seconds: int,
                 implementation_commit: str, range_config: ConfigV441,
        ...
        if not acknowledge_construction_only:
            raise ConfigNotRatifiedErrorV43(
                "RangeSemanticEngineV441 e ADITIV, NEIMPLEMENTAT-IN-PRODUCTIE -- constructia refuza fara "
                "acknowledge_construction_only=True explicit (mandat: nicio promovare Strategy Catalog/"
                "Alpha/AI Trader/LIVE_SHADOW/broker autorizata de acest mandat)")
        if range_config.config_id() != RANGE_HIERARCHICAL_V4_4_1_NORMATIVE_CONFIG_ID:
            raise ContractErrorV43(
                f"CONFIG_ID_MISMATCH: {range_config.config_id()} != normativ "
                f"{RANGE_HIERARCHICAL_V4_4_1_NORMATIVE_CONFIG_ID}")
        ...
        self._range = RangeSemanticProducerV441(self._cfg)
        self._symbol = symbol
    ...
    @property
    def range_config(self) -> ConfigV441:
        return self._cfg
    ...
    def observe_closed_bar(self, bar: Any, *, as_of: int | None = None
                           ) -> tuple[Any, RangeSemanticResultV44, list[RangeEventV43]]:
    ...
    def replay_batch(self, bars: Any, *, as_of: int | None = None) -> RangeLedgerV441:
        records: list[RangeReplayRecordV441] = []
        ...
        for bar in bars:
            ...
            records.append(RangeReplayRecordV441(
    ...
    def snapshot(self) -> RangeSnapshotV441:
        return RangeSnapshotV441(
    ...
    def restore(self, snapshot: Any) -> None:
        if not isinstance(snapshot, RangeSnapshotV441):
            raise RangeSnapshotErrorV441(
                f"refuz: snapshot de tip {type(snapshot).__name__!r}, nu RangeSnapshotV441 -- nicio migrare "
                "implicita de la V4.4 sau alta versiune, niciun rebuild partial acceptat")
        if snapshot.contract_version != self._cfg.contract_version or snapshot.config_id != self._cfg.config_id():
            raise RangeSnapshotErrorV441(SNAPSHOT_CONTRACT_MISMATCH)
        if snapshot.n1_identity_fingerprint != self._n1.identity.fingerprint():
            raise RangeSnapshotErrorV441("identitate N1 incompatibila")
        ...
        try:
            ...
            raise RangeSnapshotErrorV441(f"snapshot range_state corupt/incomplet: {exc!r}") from exc
