"""Benchmark RangeStateReplayEngine (0.2.0) — cost per-bară MĂRGINIT (N1 O(1)/460 + RANGE O(range_window)).

Datele reale XAUUSD M15 sunt SIGILATE ⇒ serie sintetică deterministă de aceeași dimensiune, cu structură de range
(oscilații) + breakout-uri, ca toate căile RANGE/eveniment să fie exercitate. Măsoară wall/CPU/bars-sec/scaling/mem
+ overhead snapshot/restore. Țintă: sub 4h la 355.696 bare, scalare ~liniară.
"""
...
def synth_bars(n: int) -> list:
    """Oscilație determinstă (range) cu breakout-uri periodice — fără Date/random (deterministă din index)."""
    bars = []
    base = 2400.0
    for i in range(n):
        cyc = i % 40
        # 32 bare de oscilație (range), 8 bare de breakout
        if cyc < 32:
            phase = (cyc % 8)
...
def bench(n: int) -> dict:
    bars = synth_bars(n)
    cfg = RangeConfig(d_min_bars=10)
    eng = RangeStateReplayEngine(symbol="XAUUSD", timeframe="15m", bar_interval_seconds=900,
    ...
    eng2 = RangeStateReplayEngine(symbol="XAUUSD", timeframe="15m", bar_interval_seconds=900,
                                  implementation_commit=IC, range_config=cfg)
