"""Selectie determinista `V4_4_1_FRESH_BLIND14`, conform protocolului comis la `4af8ea9`.
...
SEED_STRING = b"RANGE_V4_4_1_FRESH_BLIND14|4ed4eb4|6adef91|N14"
...
def main() -> int:
    ...
    # E8 — cele 14 ferestre FB14: dovezi de diagnostic CONSUMATE, nu se reutilizeaza.
    before = int(ex.sum())
    fb = json.load(io.open(os.path.join(ESCROW_DIR, "selection_fb14.json"), encoding="utf-8"))
    for w in fb["windows"]:
        ex[max(0, w["r_start"] - INDEP):min(n, w["r_end"] + INDEP)] = True
    acc["E8_FB14_14_windows"] = int(ex.sum()) - before
    ...
    if len(order) != 3:
        print(f"F441_BLOCKED_UNEXPECTED_ELIGIBLE_BLOCKS={len(order)}")
        return 1
    ...
    for L, _ in LENGTH_QUOTA:                       # lungimi descrescatoare
        for bi in order:                            # blocuri, capacitate descrescatoare
            for _k in range(plan[(bi, L)]):
                pool = free(L, bi)
                if not pool:
                    print(f"F441_BLOCKED_NO_FEASIBLE_{blocks[bi][0]}_L{L}")
                    return 1
    ...
    out = {
        "batch_id": "V4_4_1_FRESH_BLIND14",
        "epistemic_status": "FRESH_BLIND14_CONFIRMATION",
        "protocol_commits": ["4af8ea9"],
        "authority": {"rt_audit": "6adef91", "v4_4_1_implementation": "4ed4eb4",
                      "v4_4_reference": "3bb61cf", "config_id_prefix": "d7b6c067"},
    ...
    io.open(os.path.join(ESCROW_DIR, "selection_f441.json"), "w",
