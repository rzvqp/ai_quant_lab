...
| M15-F2-SESSION2LEG | does a session impulse->reset->second-leg follow through (London/NY)? | E (session structure) | **CLOSED_NO_ROBUST_ALPHA** — all 4 (London/NY x L/S) dead (WRt 0.02-0.16, best10<0, reset stop 39-54p noise-stopped) |
| M15-F3-BRKACCEPT | does break->ACCEPTANCE->first-retest continue (acceptance filters false breaks)? | D (break+accept+retest) | **CLOSED_NO_ROBUST_ALPHA** — dead both sides (WRt 0.03-0.06, retest stop 19-20p noise-stopped, best10<0). Acceptance filter did not overcome the tight-stop wall |

**HISTORICAL_M15_PRICE_ONLY_FRONTIER_EXHAUSTED (structural):** 3 genuinely-distinct M15 classes (F1 displacement-pullback, F2 session-2nd-leg, F3 break-accept-retest) all NOT robust with the SAME signature (WRt 0.03-0.22, best10<0, tight intraday stops noise-stopped). Decisive dilemma: an M15 mechanism uses either TIGHT intraday stops (noise-stopped -> dead) OR WIDE swing stops (-> REDUNDANT with the frozen H4 edges). No NEW non-redundant robust price-only alpha on M15. `EXOGENOUS_FRONTIER_REMAINS_NEXT_HIGH_PRIORITY_OPTION`.
