...
CURRENT_FRONTIER          = ARCHITECTURE RESET: mode->event->specialist. Cycle 1 froze MARKET_OPERATING_MODE_V1 (price-only causal H4, 6 modes, separates PRIMARY_BEAR from BULL_CORRECTION). Population stable + sufficient N all eras.
CURRENT_HYPOTHESIS        = -
...
NEXT_FRONTIER             = FIRST structural-event family (§10): LIQUIDITY EVENT -> RECLAIM/FAILURE -> DISPLACEMENT -> FUTURE PATH, M15 events conditioned on frozen H4 mode, information-first decomposition
```
