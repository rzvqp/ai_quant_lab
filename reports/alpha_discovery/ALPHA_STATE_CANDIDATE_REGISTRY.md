...
- **Status:** `INFO_CROSS_STABLE_PENDING_CHARACTERIZATION`. NOTE: SAME DOWN-H4-short-continuation MECHANISM as ST-H4DN-M15DNIMP-SHALLOW-SHORT (different trigger). NEXT: §14-15 structural-stop tradeability net STRESS cross-era + §17 REDUNDANCY check (event-day overlap vs F2 impDn-shallow). If tradeable+non-redundant -> freeze; if era-conditional (per F2 prior) or redundant -> record, not frozen.
