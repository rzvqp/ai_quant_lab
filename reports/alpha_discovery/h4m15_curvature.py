"""h4m15_curvature.py — Family 5 (last univariate class): M15 TIME-SINCE-EXTREME (fresh window high/low = momentum)
+ path CURVATURE/ACCELERATION (recent velocity vs prior velocity), conditional on causal H4 state (§5).
freshHigh/Low = current bar makes the 8-bar extreme (new-momentum). accel/decel = 4-bar velocity now vs prior
(accel=continuation, decel=exhaustion). P(+70/-50 & +100/-70) lift vs SAME-H4-STATE base, L/S separate,
event-deduped, same-H4-state cross-era gate. Interpretable only (§7).
"""
...
def descr(df):
    ...
    hh=h.rolling(W).max().to_numpy(); ll=l.rolling(W).min().to_numpy()
    freshHigh=h.to_numpy()>=hh; freshLow=l.to_numpy()<=ll
    v_rec=(c-pd.Series(c).shift(4).to_numpy())/atr; v_pri=(pd.Series(c).shift(4).to_numpy()-pd.Series(c).shift(8).to_numpy())/atr
    return {
      "freshHigh(mom)": freshHigh, "freshLow(mom)": freshLow,
      "accelUp":  (v_rec>0.3)&(v_rec>v_pri),   # up-move accelerating -> continuation
      "decelUp":  (v_rec>0.3)&(v_rec<v_pri),   # up-move slowing -> exhaustion
      "accelDn":  (v_rec<-0.3)&(v_rec<v_pri),  # down-move accelerating -> continuation
      "decelDn":  (v_rec<-0.3)&(v_rec>v_pri),  # down-move slowing -> exhaustion
    }

def main():
    print("FAMILY 5: M15 fresh-extreme (momentum) + curvature/accel conditional on H4 state. P lift vs SAME-H4-STATE base, deduped, cross-era.")
        ...
        for dname,cond in D.items():
            ...
            for (X,Y) in TARGETS:
                for side in ('L','S'):
                    ...
                    print(f"   {dname:15s} {side} +{X}/-{Y}: base={base:.2f} lift={lift:+.3f}(EffN {nE}) {ds} yr[{pys}]"+(" <== CROSS_STABLE" if stable else ""))
