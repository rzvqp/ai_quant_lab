    ...
    manifest: dict[str, Any] = {
        ...
        "lm_001_preregistration": {
            ...
            "execution_layer": {
                ...
                "displacement_filter": {
                    "status": "REOPENED (was RATIFIED) -- see cost_constant_correction_v2_7_8. Derived on BOTH the wrong cost (0.40 vs corrected 0.20) AND the wrong TICK-as-pip-divisor (0.10 vs corrected 0.01) simultaneously -- the compound case. Fresh floor approx 58 pips (real $0.58, not $1.00); old ceiling (65 pips = real $6.50) relabels to approx 650 pips at corrected TICK, a pure unit relabeling not requiring fresh geometry. Both must be re-derived from raw geometry, not hand-adjusted on these old percentiles.",
            ...
            "FINAL_VERDICT": {
                "status": "REJECTED_NET_OF_COST -- REOPENED (Mandate 3.22, cost_constant_correction_v2_7_8): this verdict was reached entirely on cost_round_trip=0.40, now corrected to $0.20. All figures below (expectancy_R, net_sumR, gross_edge_R_est, p_wp5) were computed on the wrong cost and must be re-run, not hand-adjusted -- kept here UNEDITED as the historical record of what was actually measured, per standing documentation discipline (mark REOPENED, never silently delete or patch numbers in place).",
        ...
        "next_hypothesis_atr_partial_exit_v2_7_8": {
            "status": "PRE-REGISTERED, AWAITING VALIDATION_ENGINE_CODE -- not yet named in the SMC_S* grammar (needs a new namespace decision before implementation, per the already-established naming discipline)",
            "bias": "H1 and H4 trend (h1_trend_up/h4_trend_up, code/mtf.py, ema20>ema50 -- verified to exist exactly as claimed).",
            ...
            "stop_and_targets_ATR_multiples": {
                "status": "CHOSEN, NOT DERIVED -- stated explicitly per CEO's own framing (accept as pre-registered design, or derive; Statistician accepts as chosen)",
            ...
            "eligibility_filter": {
                "status": "OLD [10.1,65.0) PIPS FILTER ELIMINATED -- wrong on two independent counts: derived on the uncorrected cost, AND assumes a structural (displacement-based) stop, not this hypothesis's ATR-based stop.",
                ...
                "CRITICAL_FLAG_hypothesis_threatening": "ATR_min (approx 86 pips) EXCEEDS even the CEO's own cited CURRENT typical M15 ATR (approx 74 pips, at gold approx $4050) -- and far exceeds the historical discovery-period median (approx 17-18 pips, 2011-2021). This suggests the eligible population on the existing M15_v2 discovery data may be minuscule or empty at the corrected $0.20 cost. NOT a conclusion -- a pre-execution finding requiring the actual eligible-bar-count to be verified directly (not assumed) before any run. If the population proves insufficient, options include: a tighter cost estimate (CEO's stated low end, $0.05 spread -> cost approx $0.10 -> ATR_min approx 43 pips), restricting to the recent high-ATR regime only (raising n-sufficiency concerns), or accepting a small population under the lab's existing n>=25 convention.",
            },
            "five_criteria_status": {
                "numeric_threshold": "H0: mu_netR<=0 (unchanged conceptually from LM-001's convention)",
                "horizon_as_bars": "VARIABLE (see horizon_variable_not_fixed) -- explicit deviation, reported as a distribution",
                "population": "PENDING -- decided by whether the ATR_min eligibility floor leaves a testable population (see eligibility_filter.CRITICAL_FLAG)",
                "classification_threshold": "alpha=0.05, family of 1 (structurally distinct from SMC_S1-S20, new namespace pending)",
                "zero_free_parameters": "All elements above fixed or explicitly declared as chosen (ATR multiples) -- no parameter left silently undecided",
            },
            "full_resolution_document": "ai_quant_lab statistician/STATISTICIAN_COST_CONSTANT_CORRECTION_OB_DEMANDZONE_HYPOTHESIS_v1.0.md",
        },
        "timeframes": {
