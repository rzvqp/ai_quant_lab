...
today, since `ve_tower` genuinely is not installed.

This stub exists solely so integration tests can exercise the full "handshake ACCEPTED" path against a
REAL worker subprocess without installing `ve_tower` -- it returns a fixed, clearly-synthetic identity that
a matching TEST pin (`ai_trader/new_brain_bridge/tests/_tower_test_pin.py`) is written to accept. Only
activated when the worker process is launched with the environment variable
...
from ve_tower_worker.artifact_identity import WORKER_BUILD_COMMIT, WORKER_PACKAGE_VERSION
