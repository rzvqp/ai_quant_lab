...
def _null_means(x_centered: np.ndarray, n: int, L: int, B: int, rng) -> np.ndarray:
    ...
    return x_centered[idx].mean(axis=1)
...
def run(series, block_length: int, B: int, tail: str = "right",
        centering: str = "zero", *, seed: int) -> dict:
