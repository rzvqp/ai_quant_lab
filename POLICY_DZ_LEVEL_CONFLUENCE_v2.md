# POLICY — Demand-Zone Re-entry × PDH/PDL Confluence — **v2.0 (Part B completed, screening)**

# 🟡 SCREENING_BASELINE · NOT STATISTICALLY VALIDATED · NOT PRODUCTION APPROVED

**CAND-0019.** One frozen, executable structural Part B for identical screening (CEO order). Single
...
`POLICY_DZ_LEVEL_CONFLUENCE_v1.md`). **No new primitive** — v1.0 W10 block stands (`8edbf99`:
...
Family = two-structure confluence containing a daily level (demand zone × level).

| Field | Method · reason |
|---|---|
| **stop_loss** | **Below BOTH — the deeper floor:** long → `min(DemandZone.zone_lower, low[touch_idx])`; short → `max(DemandZone.zone_upper, high[touch_idx])`. **Reason:** the confluence holds until both the zone far edge and the level-test bar are broken. |
