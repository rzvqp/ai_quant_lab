...
> **⚠ CORRECTION (LEDGER [31]/E31, 2026-07-30; RT-CODE-A-0006).** W9's "still OPEN on the official line" is **stale as of 2026-07-25** and the framing conflated two objects. Git-verified: the flow-c D3 fix was **cherry-picked** onto `statistician-foundation` on 2026-07-25 (`d4fb426`/`4259382`/`d4ee4bb`, new SHAs); `code/matched_null.py` and `code/mn_adversarial.py` are now **byte-identical to flow-c** (ATR-rescaling + `drift_long`/`trend_short` battery), the result JSONs **PASS** (`ALL_SCENARIOS_CALIBRATED=true`, drift `fpr05=0.0`), and `PROJECT_AUDIT.md:8` marks **D3 = RESOLVED**. W9 read "open" because its verification keyed on the **original SHAs** (still uncontained) and **flow-c not merged** (still literally true) — **both blind to a cherry-pick.** The "F6 lacks drift-beta control" is true of the **VE's separate `validation_engine/` F6 reconstruction** (a *different* test object — DC event-cell, not strategy Test B), **not** of `code/matched_null.py`. **Corrected residuals that DO survive:** (a) the passing JSONs are cherry-picked, **never independently re-run** on the official line; (b) the VE-F6 object still lacks the drift mechanism; (c) **SCOPE GAP (larger than W9): the matched-null is validated ONLY for 1.5×ATR stops — structural-stop families, which every current SMC candidate uses, were never in the calibration battery.** Append-only, not rewritten; the substance moves from "fix absent" to "fix present but un-rerun + out-of-domain for the actual candidates." Full attack: `policy_reviews/RT-CODE-A-0006_statistical_stack.md`.

**W11 — MK-01/MK-02: D2 equality rejection is a *selective* filter, not neutral.** *(Finding; LEDGER [17]/E17.)* `detect_swings` strict-`>`-both-sides discards every plateau/equal-extreme, which are exactly the equal-high/low **liquidity-pool** structures MK-02 models — so `build_pools` can never emit an equal-high pool. Regime-dependent (worst at low ATR / 2-decimal gold: 24.8/42.9/59.7% lost), confounding the loss with the phenomenon under study. Code is lookahead-safe and correct; the ratification's "cost" is understated. A standing interpretive condition on all MK-01/MK-02 output, not a bug. Full attack: `policy_reviews/RT-CODE-A-0001_MK01_MK02.md` §1.

**W12 — MK-01 `detect_breaks`: consumption cascade emits spurious over-counted breaks.** *(Defect; LEDGER [17]/E17.)* Consuming the live same-label reference re-exposes an OLDER superseded same-label swing, which emits a spurious break on a later bar (reachable with 2 ascending HHs). Over-counts structural breaks in trends/breakouts and corrupts downstream break statistics. **Not** lookahead/leakage/crash, and literal D7 is satisfied — so Stage-1 (7/7 fidelity) and Stage-2 (12/12 executability/leakage) pass it by construction; it needs a semantic test. It is a **D7 ambiguity** (is a superseded same-label swing still a live break reference?) resolved the over-counting way. Red Team flags it directly: **ratification should not proceed on `detect_breaks` until the reading is decided.** No remedy designed (would touch a ratified rule). Full attack: `policy_reviews/RT-CODE-A-0001_MK01_MK02.md` §3.

**W10 — Policy grounding not co-located with the policy (branch split).** *(Integrity risk; LEDGER [16]/E16, 2026-07-25.)* PDH/PDL policy v1.1 (commit `78634d5`, `alpha-automation-v1`) Part A is "grounded in" `code/institutional_levels.py`, but that file is **not in `78634d5`'s tree and not on `alpha-automation-v1`** — it exists **only on `discovery-mk-matrix-v1`** (verified: `git branch --contains 1930467` → `discovery-mk-matrix-v1`, not the policy's branch). Same failure mode as Part B's nonexistent v8.5 M_031–M_034; sibling of W9. **Not a mechanism defect** (the primitives exist and are correct where they live, and passed the lookahead attack), but the policy's "built on ratified primitives in the repo" is false on its own branch — a Statistician on `alpha-automation-v1` finds the grounding absent. **Blocks the Part-A handoff until resolved (architecture/merge matter, not Red Team's to design).** Full attack: `policy_reviews/RT-POLICY-A-0001_PDH_PDL_v1.1.md`.
