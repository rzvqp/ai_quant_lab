...
- **ve_tower** — official N1–N4 phenomena provider.
  ...
  - **0.5.0** (adds chain orchestrator; contracts N2/N3/N4 unchanged) — `run_tower_chain` runs N2→N3→N4 internally; caller CANNOT supply `n2_fingerprint`/`bias_available` (structural + `parse_chain_request` rejects unknown fields). `run_n3/run_n4` = `UNBOUND_DIRECT_API` (forbidden in production). `TOWER_CHAIN_BINDING_VERSION=tower-chain-binding-v1`. Closes N2_CHAIN_BINDING. 68 tests. Awaiting Red Team N2_HANDOFF / N2_CHAIN_BINDING.
